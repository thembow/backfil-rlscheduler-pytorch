...
class HPCEnv(gym.Env):
    ...
    def getDelayStart(self):
        ...
        expected_wait = (earliest_start_time - rjob.submit_time) 
        test_start = earliest_start_time + (expected_wait * 0.03)
        #print(f"debug! expected_wait={expected_wait}, earliest_start_time={earliest_start_time}, rjob.submit_time={rjob.submit_time}, rjob.request_number_of_processors={rjob.request_number_of_processors}")
        return test_start
    ...
    def build_observation(self):
        ...
        if self.backfilling and len(self.visible_jobs) > 0:
            predicted_start = self.getDelayStart()
            #old_len = len(self.visible_jobs)
            self.visible_jobs = [x for x in self.visible_jobs if (self.current_timestamp + x.request_time) < predicted_start]
        #     print(f"debug! new visible jobs len={len(self.visible_jobs)}, predicted_start={predicted_start}, self.rjob={self.rjob}, backfillable={old_len}")
