...
class HPCEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.scheduled_scores.append(sum(self.schedule_curr_sequence_reset(self.sjf_score)[0].values()))
        self.scheduled_scores.append(sum(self.schedule_curr_sequence_reset(self.f1_score)[0].values()))
    ...
    def moveforward_for_resources_backfill_greedy(self, job, scheduled_logs):
        ...
        #earliest_start_time = earliest_start_time + (expected_wait * )
        

        while not self.cluster.can_allocated(job): 
            #best fit key: key=lambda _j: self.node_score(_j)
            #fcfs key: key=lambda job: self.fcfs_score(job)
            self.job_queue.sort(key=lambda _j: self.node_score(_j))
            ...
            for _j in job_queue_iter_copy:
                if (self.current_timestamp + _j.request_time) < earliest_start_time:
                    ...
                    for running_job in self.running_jobs:
                        ...
                        if free_processors >= _j.request_number_of_processors:
                            break
                    expected_wait = (earliest_start_time - _j.submit_time) if (earliest_start_time - _j.submit_time) > 0 else 1
    ...
    def schedule_curr_sequence_reset(self, score_fn):
        ...
        return [scheduled_logs, delay_data]
    ...
    def schedule_curr_sequence(self, score_fn):
        ...
        while True:
            ...
            # if selected job needs more resources, skip scheduling and try again after adding new jobs or releasing some resources
            if not self.cluster.can_allocated(job_for_scheduling):
                print(f'debug! cannot allocate {job_for_scheduling}, backfilling now!')
                if self.backfil:
            ...
            scheduled_logs[job_for_scheduling.job_id] = score
            self.job_queue.remove(job_for_scheduling)
            print(f'debug! job scheduled heuristically={job_for_scheduling}')
    ...
    def moveforward_for_resources_backfill_modified(self, rjob, job_for_scheduling):
        ...
        #moving code out here from self.allocate for ease of understanding. this gives us the amount of nodes we need
        temp_job.allocated_machines = [[x for x in range(temp_request_nodes)]]
        #when we are doing the calculation it only uses the length of allocated machines, not the actual internal info, so just make a dummy list of nodes length
        temp_job.scheduled_time = self.current_timestamp
        temp_running_jobs.append(temp_job)
        #set up the temp job then add to the queue

        new_earliest_start_time = self.current_timestamp
        temp_running_jobs.sort(key=lambda running_job: (running_job.scheduled_time + running_job.request_time))
        free_processors = (self.cluster.free_node - temp_request_nodes) * self.cluster.num_procs_per_node
        for running_job in temp_running_jobs:
            free_processors += len(running_job.allocated_machines) * self.cluster.num_procs_per_node
            new_earliest_start_time = (running_job.scheduled_time + running_job.request_time)
            if free_processors >= rjob.request_number_of_processors:
                break
        ...
        if delay >= 3:
            self.bf_skips += 1
            print(f"DEBUG! delay={delay}, too high for backfilling {temp_job}, skipping schedule")
        ...
        while not self.cluster.can_allocated(rjob):
            ...
            if self.cluster.can_allocated(_j):
                ...
                self.job_queue.remove(_j)  # remove the job from job queue
                print(f"debug! job backfilled = {_j}")
                #setup for calculating delay
                new_earliest_start_time = self.current_timestamp
                self.running_jobs.sort(key=lambda running_job: (running_job.scheduled_time + running_job.request_time))
                free_processors = self.cluster.free_node * self.cluster.num_procs_per_node
                for running_job in self.running_jobs:
                ...
                print(f"DEBUG! cannot allocate {_j}, moving forward!")
                # move to the next timestamp
                assert self.running_jobs
                self.running_jobs.sort(key=lambda running_job: (running_job.scheduled_time + running_job.run_time))
                next_resource_release_time = (self.running_jobs[0].scheduled_time + self.running_jobs[0].run_time)
