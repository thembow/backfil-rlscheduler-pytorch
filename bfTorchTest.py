...
class HPCEnv(gym.Env):
    ...
    def schedule_curr_sequence_reset(self, score_fn):
        ...
        self.visible_jobs = []
        self.pairs = []
        #print(f"jobs backfilled during sequence={self.total_backfills}")
        self.current_timestamp = self.loads[self.start].submit_time
        self.job_queue.append(self.loads[self.start])
        self.last_job_in_batch = self.start + self.num_job_in_batch
        self.next_arriving_job_idx = self.start + 1
    ...
    def canBackfill(self):
        bf_jobs = self.visible_jobs = [x for x in self.visible_jobs if self.cluster.can_allocated(x)]
        if len(bf_jobs) > 0:
            return True
        return False
    ...
    def moveforward_for_resources_backfill_modified(self, rjob, job_for_scheduling):
        ...
        if not self.canBackfill():
            #print(f"debug! no jobs available for backfill! skipping")
        ...
        while not self.cluster.can_allocated(rjob):
            ...
            #print(f"debug! attempting to backfill {_j}")
            if self.cluster.can_allocated(_j):
                # we should be OK to schedule the job now
                #print(f"debug! job backfilled = {_j}")
                assert _j.scheduled_time == -1  # this job should never be scheduled before.
                #print(f"debug! {_j} backfilled!")
    ...
    def step_for_test(self, a):
        job_for_scheduling = self.pairs[a][0]
        if self.backfilling:
            #print(f"debug! job to backfill={a}")
