...
def run_policy(env, get_action, nums, iters, score_type, enable_preworkloads=False):
    ...
    for iter_num in range(0, iters):
        ...
        f1_r.append(sum(env.schedule_curr_sequence_reset(env.f1_score)[0].values()))
        # f2_r.append(sum(env.schedule_curr_sequence_reset(env.f2_score).values()))
        uni_r.append(sum(env.schedule_curr_sequence_reset(env.uni_score)[0].values()))
        wfp_r.append(sum(env.schedule_curr_sequence_reset(env.wfp_score)[0].values()))
        sjf_r.append(sum(env.schedule_curr_sequence_reset(env.sjf_score)[0].values()))
