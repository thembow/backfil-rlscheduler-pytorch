...
class HPCEnv(gym.Env):
    ...
    def reset(self):
        ...
        print(f"score for conservative sjf backfilling {np.mean(sum(self.schedule_sequence_con(self.sjf_score).values()))}")
        print(f"score for sjf scheduling {np.mean(sum(self.schedule_curr_sequence_reset(self.sjf_score).values()))}")
        print(f"score for f1 scheduling {np.mean(sum(self.schedule_curr_sequence_reset(self.f1_score).values()))}")
    ...
    def moveforward_for_resources_backfill(self, job):
        ...
        while not self.cluster.can_allocated(job):
            ...
            if self.next_arriving_job_idx < self.last_job_in_batch \
            and self.loads[self.next_arriving_job_idx].submit_time <= next_resource_release_time:
                #if next job isn't the last job in the queue AND next job is before next resource release time
                self.current_timestamp = max(self.current_timestamp, self.loads[self.next_arriving_job_idx].submit_time)
                self.job_queue.append(self.loads[self.next_arriving_job_idx])
                self.next_arriving_job_idx += 1
            else:
    ...
    def moveforward_for_job(self):
        """
        checks if there are jobs in the queue, then if we are at the last job in the batch.
        If there are no jobs in the queue but we aren't at the last job, adds jobs to the queue
        """
        if self.job_queue:
            return True
        ...
        while not self.job_queue:
            ...
            if self.loads[self.next_arriving_job_idx].submit_time <= next_resource_release_time:
                ...
                self.cluster.release(next_resource_release_machines)
                self.running_jobs.pop(0)  # remove the first running job.
            #if the next job arrives before the first running job ends, add it to the queue and progress time. Otherwise progress to when running job ends and remove it
        """Serves several functions
        1. Removes running jobs from the list of running jobs
        2. Releases resources
        3. Adds jobs to queue
        4. Progresses time forward

        *ideas*
        first compare next scheduled job vs next submitted job
        if scheduled first, progress time to that
        else, do the comparison of submit_time vs release_time as seen above
        but what if the scheduled time is before 
        """
    ...
    def skip_schedule(self):
        ...
        if self.running_jobs:  # there are running jobs
            ...
            next_resource_release_time = (self.running_jobs[0].scheduled_time + self.running_jobs[0].run_time)
            next_resource_release_machines = self.running_jobs[0].allocated_machines
            #sets next release time = when next running job ends

        if self.next_arriving_job_idx >= self.last_job_in_batch and not self.running_jobs:
            #if there are no running jobs and we aren't at the last job in the batch
            if not self.pivot_job:
                #not really sure what pivot job refers to?
                self.pivot_job = True
                return False, 0
            else:
                return False, 0
        ...
        if next_time_after_skip < min(self.loads[self.next_arriving_job_idx].submit_time, next_resource_release_time):
            #if in 60 seconds we are still before the next jobs scheduled time or the next release time
            self.current_timestamp = next_time_after_skip
            return False, 0
        
        if self.next_arriving_job_idx < self.last_job_in_batch and self.loads[self.next_arriving_job_idx].submit_time <= next_resource_release_time:
            #if we arent at last job in queue and the next job's submit time is before the next release time
            self.current_timestamp = max(self.current_timestamp, self.loads[self.next_arriving_job_idx].submit_time)
            self.job_queue.append(self.loads[self.next_arriving_job_idx])
            self.next_arriving_job_idx += 1
        else:
    ...
    def moveforward_conservative(self, score_fn):
            ...
            while not self.job_queue:
                ...
                if self.loads[self.next_arriving_job_idx].submit_time <= next_resource_release_time:
                    ...
                    if len(self.running_jobs) > 1:
                        print(self.running_jobs[0].job_id)
        ...
        #section above handles moving time forward when queue is empty
            ...
            # if self.loads[self.next_arriving_job_idx].submit_time <= proc_map_next_start and not out_of_jobs:
            #     #move time forward and add job to queue
            #     self.current_timestamp = max(self.current_timestamp, self.loads[self.next_arriving_job_idx].submit_time)
            #     print(f"debug! moving time forward to add jobs to queue, new timestamp = {self.current_timestamp}")
            #     self.job_queue.append(self.loads[self.next_arriving_job_idx])
...
if __name__ == '__main__':
    ...
    '''
    parser = argparse.ArgumentParser()
    parser.add_argument('--workload', type=str, default='./data/SDSC-SP2-1998-4.2-cln.swf')  # RICC-2010-2 lublin_256.swf SDSC-SP2-1998-4.2-cln.swf
...
"""
TODO:
****figure out when to run anchored jobs in step() loop - DONE
1. jobs are scheduled the moment they arrive in the queue

* make moveforward_for_job_con() DONE
* add section in schedule_con to skip to and execute anchored jobs DONE
* draw the rest of the owl 
* get cbackfilling functional
* figure out how to determine when a job ends
...
"""
    ...
    schedule list = list of job start times containing: job id, start time
    ...
    main scheduling routine
        ...
        2. update proc map 
            a. if anchor point and duration are good, add to schedule list and attach to anchor in proc map
            b. if anchor point and duration are not good, add to schedule list and attach to the end of the proc map
