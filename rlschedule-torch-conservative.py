...
class ProfileNode:
    def __init__(self, start_time, end_time, processors):
...
class ConservativeBackfill:
    def __init__(self):
        self.job_queue = []
        self.running_jobs = []
        self.profile_head = ProfileNode(0, float('inf'), 0)
    ...
    def check_availability(self, current, start_range, end_range, requested_proc):
        ...
        while current is not None and current.start_time < end_range:
            ...
            if (current.start_time <= start_range < current.end_time) or (start_range <= current.start_time < end_range):
    ...
    def update_profile(self, anchor, job):
        current = self.profile_head
        while current is not None and current.start_time < anchor:
            current = current.next

        #if current.start_time == anchor:
            #current.available_processors -= job.processors
        #else:
        new_node = ProfileNode(anchor, current.end_time, job.request_number_of_processors)
        new_node.next = current.next
        current.end_time = anchor
        current.next = new_node

    def anchor_job(self, job):
        anchor = self.find_anchor_point(job)
        if anchor is not None:
            self.update_profile(anchor, job)
            return True
        return False

    def schedule_con(self, job_for_scheduling):
        if not self.cluster.can_allocated(job_for_scheduling):
                if not anchor_job(job_for_scheduling):
                    self.skip_for_resources(job_for_scheduling)
        #add some sort of section here to skip to the next anchored job and set it as job_for_scheduling
        assert job_for_scheduling.scheduled_time == -1  # this job should never be scheduled before.
        job_for_scheduling.scheduled_time = self.current_timestamp
        job_for_scheduling.allocated_machines = self.cluster.allocate(job_for_scheduling.job_id, job_for_scheduling.request_number_of_processors)
        self.running_jobs.append(job_for_scheduling)
    ...
    #not really worried about this until we have normal cbackfilling working 
    # def compress_schedule(self, current_timestamp, free_processors):
    #     compressed_job_queue = []
    #     for job, _ in self.job_queue:
    #         anchor = self.find_anchor_point(job, current_timestamp, free_processors)
    #         if anchor is not None:
    #             self.update_profile(anchor, job)
    #             compressed_job_queue.append((job, anchor))
    #         self.job_queue = compressed_job_queue

    # def job_finished(self, finished_job, current_timestamp, free_processors):
    #     self.running_jobs.remove(finished_job)
    #     free_processors += finished_job.processors
    #     self.compress_schedule(current_timestamp, free_processors)
...
* make skip_for_resources_con()
* make moveforward_for_job_con()
* make skip_schedule_con()
* add section in schedule_con to skip to and execute anchored jobs
